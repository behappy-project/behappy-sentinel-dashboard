...
public class DegradeController {
    ...
    public Result<DegradeRuleEntity> apiUpdateRule(@PathVariable("id") Long id,
        ...
        try {
            List<DegradeRuleEntity> rules = ruleProvider.getRules(entity.getApp());
            DegradeRuleEntity oldEntity = rules.stream().filter(item -> item.getId().equals(id)).limit(1).collect(toList()).get(0);
            ...
            logger.error("Failed to save degrade rule, id={}, rule={}", id, entity, throwable);
            return Result.ofThrowable(-1, throwable);
        }
        return Result.ofSuccess(entity);
    }
